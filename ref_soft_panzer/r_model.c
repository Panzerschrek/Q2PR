...
{
	int s= (color[0]+color[1]+color[2])/3;
	color[0]= (color[0] * sat + s * inv_sat)/255;
	color[1]= (color[1] * sat + s * inv_sat)/255;
	color[2]= (color[2] * sat + s * inv_sat)/255;
}
...
=================
*/
//PANZER - returns number of bytes in lightmax texel
void Mod_LoadLighting (lump_t *l)
{
	...
	/*if( strcmp(r_lightmap_mode->string, "lightmap_linear_rgbs" ) == 0 )
	{
		...
		else if( r_lightmap_saturation->value < 0.0f )
			ri.Cvar_Set( "r_lightmap_saturation", "0.0" );
		...
		for (i=0 ; i<size ; i+=4, in+=3)
		{
			...
			if( loadmodel->lightdata[i+1] > max_component )
				max_component= loadmodel->lightdata[i+1];
			if( loadmodel->lightdata[i+2] > max_component )
				max_component= loadmodel->lightdata[i+2];
			if( max_component == 0 )
				max_component= 1;

			loadmodel->lightdata[i  ]= loadmodel->lightdata[i  ] * 255 / max_component;
			loadmodel->lightdata[i+1]= loadmodel->lightdata[i+1] * 255 / max_component;
			loadmodel->lightdata[i+2]= loadmodel->lightdata[i+2] * 255 / max_component;
...
void R_BeginRegistration (char *model)
{
	...
	//registration_sequence++; //PANZER do it in PANZER_BeginRegistration

	r_oldviewcluster = -1;		// force markleafs
	Com_sprintf (fullname, sizeof(fullname), "maps/%s.bsp", model);

	//D_FlushCaches (); //PANZER renderer has no caches
